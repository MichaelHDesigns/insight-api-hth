...
[![Build Status](https://img.shields.io/travis/umbru/insight-api/master.svg?branch=master)](https://travis-ci.org/umbru/insight-api)
...
> A Umbru blockchain REST and WebSocket API Service

This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/umbru/insight-ui.
...
```bash
npm install -g umbrucore-node
umbrucore-node create mynode
cd mynode
umbrucore-node install insight-api
umbrucore-node start  # to also start the service
```
...
- [umbrucore Node Umbru 4.x](https://github.com/umbru/umbrucore-node)

**Note:** You can use an existing Umbru data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` need to be enabled in `umbru.conf`, as well as a few other additional fields.

### Query Rate Limit

To protect the server, insight-api has a built-in query rate limiter. It can be configurable in `umbrucore-node.json` with:
...
With all the configuration options available: https://github.com/umbru/insight-api/blob/master/lib/ratelimiter.js#L10-17
...
- (Medium)[How to setup a Umbru Instant-Send Transaction using Insight API - The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)

## Contributing

Feel free to dive in! [Open an issue](https://github.com/umbru/insight-api/issues/new) or submit PRs.
